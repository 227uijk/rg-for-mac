...
static void exit_handler() {
    eap_state_machine_send_logoff(); // Must run before the interface is torn down
    free_if_impl();
...
/*
 * SIGINT/SIGTERM may arrive while the main thread is blocked inside
 * if_impl->start_capture() (pcap_loop). Calling exit() straight from here
 * would run the full atexit cleanup - including sending an EAPOL-Logoff via
 * the same pcap handle pcap_loop() is still using - re-entering libpcap from
 * a signal handler while it is mid-call, which can hang until forcibly killed.
 * pcap_breakloop() is the one thing libpcap documents as safe to call from a
 * signal handler; it just asks pcap_loop() to return soon. The real exit()
 * (and everything atexit() runs) then happens back on the main thread, once
 * pcap_loop() has actually returned - see the end of trans_to_preparing().
 */
static void signal_handler(int signal) {
    IF_IMPL* _if_impl = get_if_impl();
    if (_if_impl != NULL) {
        _if_impl->stop_capture(_if_impl);
    } else {
        exit(0); // Never started capturing yet, nothing unsafe to interrupt
    }
}
...
int main(int argc, char* argv[]) {
    ...
    // macOS's "do shell script ... with administrator privileges" (what the
    // menu bar app uses to launch and later stop us) spawns its child with
    // SIGHUP/SIGINT/SIGTERM already blocked in the inherited signal mask.
    // signal() only sets the handler/disposition - it does not touch that
    // mask, so without this, none of these signals are ever delivered here
    // no matter what handler is installed, and only SIGKILL ever gets us:
    // no clean shutdown, no EAPOL-Logoff, no "MiniEAP 已退出" in the log.
    sigset_t _unblock_set;
    sigemptyset(&_unblock_set);
    sigaddset(&_unblock_set, SIGHUP);
    sigaddset(&_unblock_set, SIGINT);
    sigaddset(&_unblock_set, SIGTERM);
    sigprocmask(SIG_UNBLOCK, &_unblock_set, NULL);
    ...
    if (IS_FAIL(pid_lock_save_pid())) {
        PR_ERR("PID 文件写入失败，认证进程将退出");
        return FAILURE;
    }
