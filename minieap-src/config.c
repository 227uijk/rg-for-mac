...
static void print_cmdline_help() {
    ...
    PR_RAW(
        ...
        "\t--username, -u <...>\t用户名\n"
        "\t--password, -p <...>\t密码\n"
        "\t--password-env <...>\t从指定环境变量读取密码，避免密码出现在命令行参数里\n"
        "\t--nic, -n <...>\t\t要使用的网络界面名\n"
...
#define COPY_N_ARG_TO(buf, maxlen) \
    ...
    } else if (ISOPT("password-env")) {
        const char* password_from_env = getenv(argument);
        if (password_from_env != NULL) {
            chk_free((void**)&g_eap_config.password);
            g_eap_config.password = strndup(password_from_env, PASSWORD_MAX_LEN);
            unsetenv(argument);
        }
    } else if (ISOPT("nic")) {
...
RESULT parse_cmdline_opts(int argc, char* argv[]) {
    ...
    static const struct option longOpts[] = {
	    ...
	    { "password-env", required_argument, NULL, 0 },
