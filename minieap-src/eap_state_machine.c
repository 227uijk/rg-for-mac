...
// Set for the duration of eap_state_machine_recv_handler(), which pcap_loop()
// invokes directly as its own callback. exit(EXIT_FAILURE) from deep inside
// that call chain (e.g. max-failures-exceeded) is still on pcap_loop's stack;
// sending another packet through the same pcap handle from there would
// re-enter libpcap and can hang. Skip the logoff in that one case rather than
// risk it - the server already knows something went wrong on that path anyway.
static int g_in_capture_callback = 0;

// switch_to_state(EAP_STATE_PREPARING) calls trans_to_preparing(), which itself
// recursively calls switch_to_state(EAP_STATE_START_SENT) before blocking in
// start_capture(). That inner call correctly sets PRIV->state to START_SENT,
// but once start_capture() returns (e.g. via our own pcap_breakloop() on
// shutdown) the OUTER call resumes and overwrites PRIV->state back to
// PREPARING - right before exit_handler() runs. So PRIV->state cannot be
// trusted to tell "did we ever send EAPOL-Start" at exit time; track it here
// instead, set once and never reset.
static int g_ever_sent_start = 0;

/*
 * Best-effort notification to the server that we are going away, so it does
 * not keep holding our session (and rejecting the next login attempt) until
 * its own idle timeout expires. Safe to call even if we never got past
 * EAPOL-Start, or if nothing was ever initialized.
 */
void eap_state_machine_send_logoff() {
    if (PRIV->packet_builder == NULL) {
        return; // Never initialized
    }
    if (!g_ever_sent_start) {
        return; // Never sent EAPOL-Start, nothing to log off
    }
    if (get_if_impl() == NULL) {
        return; // Interface never got selected/opened
    }
    if (g_in_capture_callback) {
        PR_WARN("跳过 EAPOL-Logoff：当前仍在抓包回调栈内，此时发送可能导致 libpcap 重入卡死");
        return;
    }
    PR_INFO("正在发送 EAPOL-Logoff 通知服务器下线");
...
static RESULT state_mach_process_failure(ETH_EAP_FRAME* frame) {
    ...
    if (PRIV->state == EAP_STATE_SUCCESS) {
        ...
        pid_lock_clear_online(); // No longer truly online, whether we retry or exit next
...
void eap_state_machine_recv_handler(ETH_EAP_FRAME* frame) {
    g_in_capture_callback = 1; // Left at 1 if a failure path below calls exit(); that's fine, see the flag's comment
